...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('portraits.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('portraits.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('portraits.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
