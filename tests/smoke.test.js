...
describe('website smoke test', () => {
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/ed505641-7fab-598f-a27b-c58fb38cc49e')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/2f2d6f96-e497-5489-ae20-7c7ca9950d9b')
    ...
    const isl = await mountSite('#/item/ed505641-7fab-598f-a27b-c58fb38cc49e')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === 'ed505641-7fab-598f-a27b-c58fb38cc49e')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Portrait of the Ottoman Sultan Ahmed I." }
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/7ab5bd97-3948-5e4a-9db2-309afc5b078e')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain('National Museum of Contemporary Art – Chiado Museum')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/70c67f69-48f3-55ab-abd2-0e678dca303b')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('National Museum of Contemporary Art – Chiado Museum'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("Lisbon"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/70c67f69-48f3-55ab-abd2-0e678dca303b/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['portraits', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
